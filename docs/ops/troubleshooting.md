...
**EmailWrapper.tsx** が `<html>`, `<body>` タグを含むフルHTMLドキュメントを出力し、Next.js の `layout.tsx` が提供する `<html>`, `<body>` と二重になっているため。

Next.js App Router では、`layout.tsx` がすでに `<html>` と `<body>` を提供しているため、ページコンポーネント内で同じタグを使用すると hydration error が発生します。

### 解決策

`src/app/draft/page.tsx` で `<EmailWrapper preview={true}>` を使用してください。
...
EmailWrapper コンポーネントは、以下の2つの用途で設計されています：

1. **ブラウザプレビュー用**（Next.js App Router）
   - `preview={true}` を指定
   - `<html>`, `<body>` タグを出力しない
   - テーブルレイアウトのみ返却

2. **メール配信用**（Resend API）
   - `preview={false}`（デフォルト）または prop なし
   - `<html>`, `<body>` を含むフルHTMLを出力
   - メールクライアント互換性のため

### 参考

- **EmailWrapper.tsx**: `src/components/email/EmailWrapper.tsx`
- **使用例**: `src/app/draft/page.tsx`
- **メール配信スクリプト**: `src/scripts/send-test-email.ts`, `src/scripts/send-production-email.ts`
